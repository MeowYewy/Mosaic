...
struct RedactionRegion {
    ...
    QString kind;         // idcard / phone / name / manual / other
    QString source;       // auto / manual
    QString label;
};
...
class RedactionModel : public QAbstractListModel
{
    ...
    Q_PROPERTY(int pageFilter READ pageFilter WRITE setPageFilter NOTIFY pageFilterChanged)
...
public:
    ...
    QVector<RedactionRegion> regionsForPage(int page) const;
